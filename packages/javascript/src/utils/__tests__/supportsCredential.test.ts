...
describe('supportsCredential', (): void => {
  ...
  it('should reject an attribute that carries no metadata (not marked credential: true)', (): void => {
    expect(supportsCredential({password: {}}, 'password')).toBe(false);
  });

  it('should reject a declared attribute that is not marked credential: true', (): void => {
    const schema: Record<string, AttributeSchema> = {
      email: {type: 'string', unique: true},
    };

    expect(supportsCredential(schema, 'email')).toBe(false);
  });

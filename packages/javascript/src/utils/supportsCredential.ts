 ...
 * stored today. That second question is the server's to answer.
 *
 ...
 * @param attribute - The credential attribute to check (e.g. `password`, `pin`).
 * @returns `false` only when the schema is known and either defines no attribute named
 * `attribute` or defines one that isn't marked `credential: true`.
 * @example
 * ```typescript
...
const supportsCredential = (
  ...
  return userSchema[attribute]?.credential === true;
};

...
export class ThunderIDWebSamplePage extends GateLoginPage {
  ...
  /** Non-throwing, immediate check for whether a session is currently active — unlike
   * {@link verifyLoggedIn}, doesn't wait or assert. Used to recover a test's cleanup logic when
   * an earlier step failed partway through a sign-out/sign-in cycle and it's unclear whether the
   * page is currently authenticated. */
  async isLoggedIn(): Promise<boolean> {
    return this.page.locator(USER_DROPDOWN_TRIGGER).first().isVisible();
  }
